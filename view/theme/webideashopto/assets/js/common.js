		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					setTimeout(function () {
						jQuery('.shopping__cart__inner .shoping__total .total__price').html(json['total']);
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					setTimeout(function () {
						jQuery('#my_wishlist-total span').html(json['total']);
		...
		return this.each(function() {
			...
			this.request = function() {
				...
				this.timer = setTimeout(function(object) {
					object.source(jQuery(object).val(), $.proxy(object.response, object));
				}, 200, this);
			}
			...
			jQuery(this).siblings('ul.dropdown-menu').delegate('a', 'click', $.proxy(this.click, this));

		});
	}
})(window.jQuery);

$('#my_test').on('click', function () {
	console.log('ok');
})
